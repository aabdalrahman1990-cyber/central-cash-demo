...
export function SidebarNav({ locale, dict, mobile = false, onNavigate }: SidebarNavProps) {
  ...
  return (
    <aside
      className={
        mobile
          ? "flex h-full min-h-0 flex-col rounded-[28px] border border-white/10 bg-slate-950/90 p-4 text-white shadow-2xl"
          : "glass-panel executive-card metric-glow rounded-[30px] border border-white/50 p-5 shadow-panel"
      }
    >
      {mobile ? (
        <div className="mb-4 rounded-[22px] bg-[linear-gradient(140deg,rgba(8,47,73,0.96),rgba(21,128,61,0.92),rgba(234,179,8,0.74))] px-4 py-4 text-white">
          <p className="text-xs text-white/70">{dict.sidebar.title}</p>
          <h3 className="mt-2 text-lg font-semibold">{dict.sidebar.subtitle}</h3>
        </div>
      ) : (
      ...
      <nav
        className={
          mobile
            ? "grid min-h-0 flex-1 gap-2 overflow-y-auto overscroll-contain pr-1 pb-24"
            : "space-y-2"
        }
      >
        {navItems.map((item) => {
